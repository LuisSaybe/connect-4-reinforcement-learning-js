...
export class Game {
  ...
  hasWinner() {
    for (let column = 0; column < this.getColumns(); column++) {
      for (let row = 0; row < this.getRows(); row++) {
        if (
          this.connects(row, column, Board.YELLOW) ||
          this.connects(row, column, Board.RED)
        ) {
          return true;
        }
      }
  ...
  connects(x, y, color) {
    ...
    for (const direction of directions) {
      for (let vectorIndex = 0; vectorIndex < this.lengthToWin; vectorIndex++) {
        ...
        for (
          let vectorLength = 0;
          vectorLength < this.lengthToWin;
          vectorLength++
        ) {
          const newX =
            x + direction[0] * -vectorIndex + direction[0] * vectorLength;
          const newY =
            y + direction[1] * -vectorIndex + direction[1] * vectorLength;
          vector.push([newX, newY]);
        }

        const points = vector.filter(
          (point) =>
